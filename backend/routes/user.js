...
router.post("/sign-up", async (req, res) => {
  try {
    ...
    // check if user already exists or not with current email, find in User model:-
...
router.post("/sign-in", async (req, res) => {
  try {
    ...
    // get existing user if it exists from the db
    ...
    // create cookie of name podcasterUserToken  (name,jwtToken ,cookie options in {})
    res.cookie("podcasterUserToken", token, {
      httpOnly: true,
      maxAge: 1000 * 60 * 60 * 24 * 30, //30 days expiry  in milliseconds
      secure: process.env.NODE_ENV === "production", //The cookie will only be sent over HTTPS if the app is in production mode (i.e., secure environments).
      sameSite: "None", //Allows the cookie to be sent across different domains (important for cross-site requests).
    });
    // send response:-
...
router.post("/logout", async (req, res) => {
  ...
  // clear cookie (name,cookie options in {})
  res.clearCookie("podcasterUserToken", {
    httpOnly: true,
    secure: process.env.NODE_ENV === "production",
    sameSite: "None",
    path: "/", //Indicates that the cookie applies to the entire domain.
  });
...
// this authMiddleware will authenticate the user and set it in req.user
// use authMiddleware to get the user by a middleware in req
router.get("/user-details", authMiddleWare, async (req, res) => {
  try {
    //get email id from req.user (req.user mein authMiddleware daaldega user ko)
